...
> **Para el comité de arquitectura** — **Catálogo maestro único del Hito 1**: veintiséis aplicaciones de negocio (APP-01 … APP-26) y cuatro plataformas (Plataforma de Observabilidad Unificada (PLT-01) … Plataforma IaC (PLT-04)). **Convención obligatoria en todo el repositorio:** siempre **nombre oficial + (APP-XX)** o **nombre oficial + (PLT-XX)** juntos. Prohibido mencionar solo el ID, solo el nombre abreviado, o abreviaturas ambiguas (Azure API Management (APP-01), OMS centralizado / Orquestador de Pedidos (APP-02), bus, WMS Principal (On Premises) (APP-06), TMS (Transportation Management) (APP-11), ERP Financiero (On Premises) (APP-25) sin ID). Stack, BD y mapa tecnológico por entorno → doc `08`. Infraestructura y red → doc `07`.
...
| Pregunta | Respuesta |
|---|---|
| **¿Incluyo plataformas (PLT)?** | **Sí, si están desplegadas o parcialmente desplegadas** (ej. **Plataforma de Identidad y Accesos (IAM) (PLT-02)** Plataforma de Identidad y Accesos (IAM) con Azure AD + OAuth en **APP-01**). Son apps habilitadoras TOGAF, no APP de negocio. |
...
### Nomenclatura oficial — WMS Principal (On Premises) (APP-06)

| ID | Nombre oficial AS IS | Nombre oficial TO BE | Función |
|---|---|---|---|
| APP-06 | **WMS Principal (On Premises)** | **WMS Cloud** (reemplaza APP-06) | WMS Principal (On Premises) (APP-06) central del CD principal; SQL Server on premises |
| APP-07 | **WMS Satélite (On Premises local)** | Absorbido por **WMS Cloud** (modo degradado local) | WMS Principal (On Premises) (APP-06) local en almacenes pequeños; sync horaria con APP-06 |

> No usar indistintamente “WMS Principal (On Premises) (APP-06) / WMS Satélite (On Premises local) (APP-07)”, “WMS Principal (On Premises) (APP-06) central” o “WMS Principal (On Premises) (APP-06)” en tablas de aplicaciones: referirse siempre al nombre oficial con ID (**APP-06** / **APP-07**). “WMS Cloud” es el reemplazo TO BE a partir de F2.

### Nomenclatura oficial — OMS centralizado / Orquestador de Pedidos (APP-02) / Orquestación de pedidos

| ID | Nombre oficial AS IS | Nombre oficial TO BE | Función |
|---|---|---|---|
| APP-02 | **Orquestador de Pedidos** | **OMS centralizado / Orquestador de Pedidos** (evoluciona APP-02) | Gestiona el ciclo de vida de la orden end-to-end: validación, deduplicación, idempotencia, estado canónico, reservas, liberaciones y coordinación con WMS Cloud, TMS (Transportation Management) (APP-11) y ERP Financiero (On Premises) (APP-25) |

> No crear un nuevo ID para “OMS centralizado / Orquestador de Pedidos (APP-02)” en el Hito 1. El OMS centralizado / Orquestador de Pedidos (APP-02) es la evolución TO BE de **Orquestador de Pedidos (APP-02)**: conserva la aplicación y su plataforma base, pero amplía su responsabilidad desde coordinación técnica hacia gobierno del ciclo de vida de la orden.
...
- **COTS ⚠️** = producto comercial licenciado; el caso **no indica marca** (WMS Principal (On Premises) (APP-06), TMS (Transportation Management) (APP-11), ERP Financiero (On Premises) (APP-25)).
...
|---|---|---|---|---|
| APP-01 | Azure API Management (APP-01) | PaaS proveedor (Microsoft) | Azure | Azure API Management (APP-01) |
...
| APP-06 | WMS Principal (On Premises) | COTS o Custom ⚠️ | On Premises · SQL Server | WMS Principal (On Premises) (APP-06) principal / WMS Principal (On Premises) (APP-06) / WMS Satélite (On Premises local) (APP-07) |
| APP-07 | WMS Satélite (On Premises local) | COTS o Custom ⚠️ | On Premises local | WMS Principal (On Premises) (APP-06) local / versión local sync horaria |
| APP-08 | Control de Inventario | COTS o Custom ⚠️ inferido | On Premises | *(inferido del ERP Financiero (On Premises) (APP-25)/WMS)* |
...
| APP-11 | TMS (Transportation Management) | COTS o Custom ⚠️ | Azure | TMS (Transportation Management) (APP-11) en Azure |
...
| APP-15 | App de Conductores (APP-15) | Custom | AWS · DynamoDB | App de conductores en AWS |
...
| APP-25 | ERP Financiero (On Premises) | COTS ⚠️ | On Premises | ERP Financiero (On Premises) (APP-25) on premises |
...
>
> **Plataforma vs conectividad:** en la columna *Plataforma* van Azure, AWS, GCP, On Premises, SaaS, etc. (dónde se ejecuta o despliega la app). La **red Wi-Fi interna del almacén** no es plataforma: es **conectividad** (Fase D — ver `07_Mapa_Infraestructura.md`, *Red almacenes*). Ejemplo: App Handhelds (APP-10) → plataforma **On Premises** (dispositivo en el CD); se conecta al WMS Principal (On Premises) (APP-06) por **Wi-Fi interno**.

**Prohibido abreviar nombres oficiales:** no usar Mgmt, Svc, On-Prem, RT, Driver, Bucket sin el nombre completo del catálogo. Ejemplos incorrectos → correctos: `Azure API Mgmt` → **Azure API Management (APP-01)**; `S3 Bucket Legado` → **Bucket S3 Legado (archivos)**; `Notification Svc` → **Servicio de Notificación (SMS/Email)**; `Optimizador RT` → **Optimizador de Rutas en Tiempo Real**.

### Aplicaciones de plataforma (Plataforma de Observabilidad Unificada (PLT-01) a Plataforma IaC (PLT-04)) — capa transversal TOGAF
...
|---|---|---|
| **Aplicaciones de negocio u operacionales** | APP-01 … APP-26 | WMS Principal (On Premises) (APP-06), TMS (Transportation Management) (APP-11), portales, App de Conductores (APP-15), ERP Financiero (On Premises) (APP-25), etc. |
| **Aplicaciones de plataforma / habilitadoras** | Plataforma de Observabilidad Unificada (PLT-01) … Plataforma IaC (PLT-04) | Observabilidad, identidad, Bus de Eventos Central (PLT-03) (cuando existan como componentes desplegados) |
...
|---|---|---|---|---|---|
| Plataforma de Observabilidad Unificada (PLT-01) | Plataforma de Observabilidad Unificada | Habilitadora | A crear — **Azure Monitor + Application Insights** | ❌ **Gap** — no desplegada; monitoreo aislado por APP | Crear: observabilidad unificada cross-cloud |
| Plataforma de Identidad y Accesos (IAM) (PLT-02) | Plataforma de Identidad y Accesos (IAM) | Habilitadora | Híbrido — **PaaS Microsoft (Azure AD)** + políticas OAuth en **APP-01** (config propia) | ⚠️ **Parcial** | Completar: MFA + Zero Trust + políticas IAM |
| Bus de Eventos Central (PLT-03) | Bus de Eventos Central | Integración / middleware | A crear — **Azure Event Hubs + Azure Service Bus** (hub central en Azure — Alternativa A) | ❌ **Gap** — no existe; integraciones P2P | Crear: Bus de Eventos Central (PLT-03) multinube con colas, DLQ y replay |
| Plataforma IaC (PLT-04) | Plataforma IaC (PLT-04) | Arq. Tecnológica (Fase D) | A crear — Terraform (capacidad propia TI + repos) | ❌ **Gap** — infra manual *(ver doc 07)* | Crear: IaC multinube |
...
> **Política OAuth 2.0** no es una aplicación: es una **configuración** de **APP-01 Azure API Management (APP-01)**. **Azure AD** es el PaaS de Microsoft que **sustenta parcialmente** **Plataforma de Identidad y Accesos (IAM) (PLT-02)** — no sustituye a Plataforma de Identidad y Accesos (IAM) (PLT-02) ni convierte APP-01 en Plataforma de Identidad y Accesos (IAM) (PLT-02).
...
El mapa AS IS incluye las veintiséis aplicaciones de negocio que el caso describe como existentes, aunque estén degradadas o marcadas para deprecación, más la Plataforma de Identidad y Accesos (IAM) (PLT-02) en estado parcial. Las brechas de Plataforma de Observabilidad Unificada (PLT-01), Bus de Eventos Central (PLT-03) e Plataforma IaC (PLT-04) no se dibujan como cajas: se listan al pie como hallazgos. En cambio, componentes TO BE como WMS Cloud o el microservicio de liquidación no aparecen en AS IS porque hoy sí existe un WMS Principal (On Premises) (APP-06) / WMS Satélite (On Premises local) (APP-07) y hojas Excel — lo que falta es la versión futura, no la función. Cualquier solución nueva se documenta en la sección TO BE y en el documento de fases.

| En el mapa AS IS **sí van** | En el mapa AS IS **no van** (van en **§4 TO BE** o doc `09`) |
|---|---|
| Aplicaciones desplegadas (APP-01 … APP-26), aunque tengan ⚠️ o 🗑️ | Plataformas en brecha: Plataforma de Observabilidad Unificada (PLT-01), Bus de Eventos Central (PLT-03), Plataforma IaC (PLT-04) |
| Plataforma de Identidad y Accesos (IAM) (PLT-02) parcial — Azure AD + OAuth en Azure API Management (APP-01) | Reemplazos TO BE: WMS Cloud, Servicio de Validación, Manifiesto Digital, etc. |
| Brechas anotadas en texto al pie, sin caja | Componentes que aún no existen en producción dibujados como si fueran apps |

### Plataforma de Identidad y Accesos (IAM) (PLT-02) vs APP-01 — aclaración para la exposición
...
│  ┌──────────────────────────────────────────────────────────────────────────────────────┐  │
│  │  Plataforma de Identidad y Accesos (IAM) (PLT-02)  ⚠️ PARCIAL                          │  │
│  │  Cobertura parcial AS IS: Azure AD (PaaS) + política OAuth 2.0 en                    │  │
│  │  APP-01 Azure API Management (APP-01) (ver caja APP-01 en capa Integración)                   │  │
...
│  · Plataforma IaC (PLT-04) (detalle: doc 07)                                                   │
├──────────────────────────────────────────────────────────────────────────────────────────────┤
...
│  └─────────────────────┘  └─────────────────────┘  └───────────────────┘  └─────────────┘  │
├──────────────────────────────────────────────────────────────────────────────────────────────┤
│  CAPA INTEGRACIÓN — sin Bus de Eventos Central (PLT-03) (PLT-03) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) (brecha → TO BE §4)                     │
│  ┌─────────────────────┐  ┌─────────────────────┐  ┌───────────────────┐                   │
...
│  │  Orquestador de     │  │  WMS Principal      │  │  TMS (Transportation│  │  App de Conductores (APP-15)   │ │
...
│  │  ante WMS Principal (On Premises) (APP-06) lento     │  │  tablas en campaña  │  │                     │  │                       │ │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘  └─────────────────────────┘ │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────────────┐ │
...
│  └─────────────────────────────────────────────────────────────────────────────────────┘     │
├──────────────────────────────────────────────────────────────────────────────────────────────┤
│  CAPA ERP Financiero (On Premises) (APP-25) / FINANZAS                                                                         │
│  ┌──────────────────────────────────────────┐  ┌───────────────────────────────────────┐    │
│  │  APP-25 ⚠️                               │  │  APP-26 🗑️                            │    │
...
Comité, este es el mapa AS IS del portafolio de RutaExpress, de arriba hacia abajo por capas. Solo incluye lo desplegado hoy, más la Plataforma de Identidad y Accesos (IAM) (PLT-02) parcial. Plataforma de Observabilidad Unificada (PLT-01), Bus de Eventos Central (PLT-03) e Plataforma IaC (PLT-04) son brechas al pie del diagrama; su solución está en TO BE. ⚠️ señala riesgo documentado; 🗑️ marca deprecación propuesta.

#### Capa transversal

Solo la Plataforma de Identidad y Accesos (IAM) (PLT-02) está desplegada, e incompleta: Azure AD y OAuth en Azure API Management (APP-01). No hay MFA ni Zero Trust. Plataforma de Identidad y Accesos (IAM) (PLT-02) y APP-01 son catálogos distintos: el gateway aparece abajo, en integración. Plataforma de Observabilidad Unificada (PLT-01), Bus de Eventos Central (PLT-03) e Plataforma IaC (PLT-04) no existen aún.
...
#### Capa ERP Financiero (On Premises) (APP-25) y finanzas
...
Veintiséis apps en multinube sin estrategia unificada, identidad parcial (Plataforma de Identidad y Accesos (IAM) (PLT-02)) y tres brechas de plataforma. Quince componentes con riesgo; tres a deprecar. El TO BE — sección cuatro y documento nueve — define el camino; inversión y roadmap en el documento once. ¿Preguntas antes de pasar al TO BE?
...
> Solo **Plataforma de Identidad y Accesos (IAM) (PLT-02)** existe de forma parcial. El resto son **gaps** documentados; la creación TO BE está en §4.

| ID | Aplicación de plataforma | Tipo TOGAF | En AS IS | Estado AS IS | Criticidad | Observaciones |
|---|---|---|---|---|---|---|
| Plataforma de Observabilidad Unificada (PLT-01) | Plataforma de Observabilidad Unificada | Habilitadora | No desplegada | ❌ Gap | Crítica | Monitoreo aislado por cada APP — **crear en TO BE** |
| Plataforma de Identidad y Accesos (IAM) (PLT-02) | Plataforma de Identidad y Accesos (IAM) | Habilitadora | Sí (parcial) | ⚠️ Parcial | Alta | Azure AD (PaaS) + política OAuth 2.0 en **APP-01** Azure API Management (APP-01) |
| Bus de Eventos Central (PLT-03) | Bus de Eventos Central | Integración | No desplegado | ❌ Gap | Crítica | Integraciones P2P — **crear en TO BE** |
| Plataforma IaC (PLT-04) | Plataforma IaC (PLT-04) | Arq. Tecnológica | No desplegada | ❌ Gap | Alta | Ver `07_Mapa_Infraestructura.md` — **crear en TO BE** |
...
| APP-15 | App de Conductores (APP-15) | Custom Mobile | AWS (ECS Fargate) | ⚠️ Activo | Crítica | Android/iOS · Offline frágil · 1,200 firmas perdidas |
...
| ID | Aplicación | Tipo | Plataforma | Estado | Criticidad | Observaciones |
|---|---|---|---|---|---|---|
| APP-01 | Azure API Management (APP-01) | PaaS | Azure | Activo | Alta | Gateway de APIs para clientes externos |
...
| — | Bus de Eventos Central (PLT-03) / Message Broker | **Bus de Eventos Central (PLT-03)** (gap) | — | ❌ Gap | Crítica | **APP-06**↔**APP-11**↔**APP-15** van P2P |

**Brechas de Integración:**
- sin Bus de Eventos Central (PLT-03) (PLT-03) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) de eventos central → todas las integraciones son punto a punto y frágiles
...
| ID | Aplicación | Tipo | Plataforma | Estado | Criticidad | Observaciones |
|---|---|---|---|---|---|---|
| APP-02 | Orquestador de Pedidos | Custom (tecnología no especificada en el caso) | Azure AKS | ⚠️ Activo | Crítica | Sin backpressure · Cola descontrolada ante degradación WMS Principal (On Premises) (APP-06) |
...
| APP-15 | App de Conductores (APP-15) | AWS + DynamoDB (✅ caso) · Framework mobile no especificado | AWS + DynamoDB | ⚠️ Activo | Crítica | Offline frágil · Evidencias perdibles · Motivos texto libre |
...
**Brechas de Data:**
- sin Bus de Eventos Central (PLT-03) (PLT-03) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)): cada sistema guarda y publica sus propios eventos
- Analítica semanal (**APP-22**) → sin alertas operativas; requiere **Plataforma de Observabilidad Unificada (PLT-01)**
...
| ID | Aplicación | Tipo | Plataforma | Estado | Criticidad | Observaciones |
|---|---|---|---|---|---|---|
| APP-08 | Control de Inventario | ⚠️ Suposición: sistema complementario al WMS Principal (On Premises) (APP-06), tecnología no especificada | On Premises | 🗑️ Activo → **eliminar F2** | Alta | No refleja inventario en tiempo real · TO BE: absorbido por WMS Cloud (sin app homóloga) |
...
- Sin integración con Bus de Eventos Central (PLT-03) → agentes no ven el estado real del pedido en tiempo real
...
| ID | Aplicación | Tipo | Plataforma | Estado | Criticidad | Observaciones |
|---|---|---|---|---|---|---|
| APP-25 | ERP Financiero (On Premises) | ⚠️ Suposición: ERP Financiero (On Premises) (APP-25) COTS, marca no especificada en el caso | On Premises | ⚠️ Activo | Alta | ✅ Caso 6a F6: facturación en ERP Financiero (On Premises) (APP-25) on premises · Sin tiempo real |
...
**Brechas de ERP Financiero (On Premises) (APP-25)/Finanzas:**
...
> Aquí van los **gaps** de Plataforma de Observabilidad Unificada (PLT-01), Bus de Eventos Central (PLT-03) y Plataforma IaC (PLT-04) (y la **completitud** de Plataforma de Identidad y Accesos (IAM) (PLT-02)). En AS IS solo se anotan como brecha; **el componente desplegado se define en TO BE**.

| Capa | Acción TO BE | Componentes |
|---|---|---|
| Transversal | **Crear** Plataforma de Observabilidad Unificada (PLT-01), Bus de Eventos Central (PLT-03), Plataforma IaC (PLT-04) · **Completar** Plataforma de Identidad y Accesos (IAM) (PLT-02) | Ver iniciativas INI-02 (Integración API-First/Event-Driven) e INI-05 (Observabilidad/Seguridad/Gobierno) en doc 10/11 |
...
| Integración | Reemplazar P2P + deprecar legado | APP-04 (deprecar), APP-21 (mantener), **Bus de Eventos Central (PLT-03) (Azure Event Hubs + Service Bus)** |
...
| Data | Pasar a streaming + limpiar datos | APP-22 (streaming), APP-24 (datos limpios), eventos canónicos en **Bus de Eventos Central (PLT-03)** |
| Soporte | Deprecar legado | APP-08 **eliminar** (→ WMS Cloud), APP-14 (deprecar), APP-17 (mantener) |
| CRM de Atención al Cliente (APP-20) | Integrar con fuente única | APP-20 (adoptar taxonomía canónica + integrar **Bus de Eventos Central (PLT-03)**) |
| ERP Financiero (On Premises) (APP-25)/Finanzas | Automatizar liquidación | APP-25 (API tiempo real), APP-26 (reemplazar con microservicio) |
...
| Aplicaciones de plataforma en AS IS | 1 parcial (Plataforma de Identidad y Accesos (IAM) (PLT-02)) |
| Brechas de plataforma documentadas (gaps) | 3 (Plataforma de Observabilidad Unificada (PLT-01), Bus de Eventos Central (PLT-03), Plataforma IaC (PLT-04)) — **a resolver en TO BE** |
