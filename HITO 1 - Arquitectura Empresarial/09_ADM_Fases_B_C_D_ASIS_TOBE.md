...
| Portal B2B (Trazabilidad) — APP-18 | Reportes de estado para cliente B2B | SaaS externo | Estados inconsistentes con ERP |
...
> **Convención:** toda aplicación listada en el AS IS de cada fase debe tener disposición explícita en el TO BE de esa misma fase: **NUEVO**, **MODIFICAR**, **CONSERVAR** o **ELIMINAR**.
...
| S3 Bucket legado (APP-04) | Canal de archivos a deprecar; clientes migran a API o Portal B2B (Carga CSV/Excel) |
...
| IoT Core sensores frío (APP-09) | Integrar alertas de temperatura con WMS Cloud para bloqueo automático de despacho | AWS IoT Core |
| ERP Financiero (APP-25) | Integrar inventario valorizado en tiempo real vía API desde WMS Cloud y Event Hub | On Premises |
...
| TMS (APP-11) | Consumir y publicar estados desde Event Store; sincronización en tiempo real con app y portal | Azure |
| DynamoDB (parte de APP-15) | Pasa a réplica operacional; Event Store (Kinesis) es la fuente canónica de eventos | AWS DynamoDB |

#### CONSERVAR
| App | Motivo |
|---|---|
| Pasarela de Pagos (APP-17) | Sin cambios en F4; integración estable según el caso; sigue procesando pagos contra entrega | SaaS externo |
...
| WMS Cloud | Recibir devoluciones automáticamente desde Servicio de Excepciones vía Event Hub (reemplaza APP-06 en F2) | Azure |
...
| S3 Evidencias (APP-16) | Exponer evidencias vía API al Servicio de Liquidación; hash de integridad obligatorio | AWS S3 |
...
Toda aplicación del AS IS debe aparecer en exactamente una columna de disposición.
...
| **F1** | APP-04 S3 Bucket legado | ELIMINAR | Canal CSV/S3 deprecado |
...
| **F1** | *(nuevo)* Bus de Eventos | NUEVO | Desacopla orquestador y WMS |
...
| **F2** | APP-09 IoT Core sensores frío | MODIFICAR | Alertas integradas con WMS Cloud |
| **F2** | APP-10 App Handhelds | MODIFICAR | Modo offline + sync segura |
| **F2** | APP-25 ERP Financiero | MODIFICAR | Inventario valorizado en tiempo real vía API |
...
| **F3** | APP-12 Optimizador de Rutas (batch) | ELIMINAR | → Optimizador Rutas RT |
| **F3** | APP-13 Portal Transportistas | MODIFICAR | Manifiestos digitales + alertas RT |
| **F3** | APP-14 Sistema Impresión Manifiestos | ELIMINAR | → Manifiesto Digital |
| **F3** | APP-15 App Conductores | MODIFICAR | Manifiesto digital completo |
| **F3** | *(nuevo)* Optimizador Rutas RT | NUEVO | Reemplaza APP-12 |
| **F3** | *(nuevo)* Manifiesto Digital | NUEVO | Reemplaza APP-14 |
| **F4** | APP-11 TMS | MODIFICAR | Estados desde Event Store |
| **F4** | APP-15 App Conductores | MODIFICAR | Offline robusto, evidencias atómicas |
| **F4** | APP-16 S3 Evidencias | MODIFICAR | Hash de integridad, retención |
| **F4** | APP-17 Pasarela de Pagos | CONSERVAR | Sin cambios en esta fase |
...
| **F5** | APP-15 App Conductores | MODIFICAR | Taxonomía obligatoria de excepciones |
| **F5** | APP-18 Portal B2B (Trazabilidad) | MODIFICAR | Notificaciones de excepción en tiempo real |
| **F5** | APP-20 CRM Atención | MODIFICAR | Taxonomía canónica + Event Store |
| **F5** | WMS Cloud *(antes APP-06)* | MODIFICAR | Devoluciones automáticas vía Event Hub |
| **F5** | *(nuevo)* Servicio de Excepciones | NUEVO | Taxonomía y correlación reclamos |
| **F6** | APP-16 S3 Evidencias | MODIFICAR | API para Servicio de Liquidación |
| **F6** | APP-18 Portal B2B (Trazabilidad) | MODIFICAR | Reportes alineados con liquidación |
| **F6** | APP-22 Plataforma de Analítica (batch) | ELIMINAR | → Analítica en Streaming |
| **F6** | APP-23 Dashboards / Reportes | MODIFICAR | BigQuery streaming |
| **F6** | APP-25 ERP Financiero | MODIFICAR | API tiempo real con Servicio de Liquidación |
...
| **F6** | *(nuevo)* Portal de Conciliación | NUEVO | Visibilidad al cliente B2B |
