...
| Rol | Participación |
|---|---|
| Cliente empresarial | Envía órdenes por API, portal o archivo |
...
| Canal | API / Portal / CSV / S3 | Múltiples canales sin normalización |
...
| Portal Clientes B2B — carga manual (APP-03) | Carga CSV/Excel para clientes medianos | SaaS externo | Sin validación automática; canal legado |
| S3 Bucket legado (APP-04) | Recepción de archivos histórica | AWS S3 | Deuda técnica; sin validación ni monitoreo |
| WMS on premises (APP-06) | Confirma reserva de inventario al recibir orden | On Premises / SQL Server | Se satura bajo alta carga; bloqueo de tablas |
...
|---|---|---|---|
| Gateway APIs | Azure API Management | Azure PaaS | Sin circuit breaker ni throttling por cliente |
| Orquestador | AKS (Kubernetes) | Azure | Sin HPA, sin KEDA; no escala en campaña |
| WMS | SQL Server on premises | On Premises | Sin HA, sin auto-scaling; degradación en Cyber Days |
...
- Los WMS satélite de almacenes pequeños sincronizan cada hora. En 74 minutos de desconexión se acumularon 4,900 movimientos en conflicto que retrasaron 18,000 pedidos.
...
| Entidad | Descripción | Problema AS IS |
|---|---|---|
| Inventario | Stock por SKU, ubicación, lote, vencimiento | Múltiples fuentes de verdad (WMS central, satélites, ERP) |
| Movimiento de inventario | Entrada, salida, picking, ajuste, devolución | 2.8% genera ajuste; no siempre auditable |
| Ubicación de almacén | Pasillo, nivel, posición, tipo | Desincronización entre WMS central y satélites |
...
| App | Rol | Plataforma | Problema |
|---|---|---|---|
| WMS Principal (APP-06) | Gestiona el inventario y guía el picking | On Premises / SQL Server | Bloqueo de tablas bajo alta carga |
| WMS Satélite (APP-07) | WMS local para almacenes pequeños | On Premises local | Sincronización horaria; conflictos al reconectar |
| App Handhelds (APP-10) | Guía al picker en el almacén | Wi-Fi interno | Sin modo offline; depende del Wi-Fi interno |
| IoT Core sensores frío (APP-09) | Monitorea temperatura de cámaras refrigeradas | AWS IoT Core | Funciona bien; alertas no integradas con WMS |
| ERP Financiero (APP-25) | Conserva inventario valorizado | On Premises | No actualizado en tiempo real; solo fin de mes |
...
| Componente | Tecnología | Plataforma | Problema |
|---|---|---|---|
| WMS central | SQL Server on premises | On Premises | Sin HA ni replica hot-standby |
| WMS satélites | BD local (tipo no especificado) | On Premises local | Sync horaria; sin reconciliación automática |
| Red almacenes | Wi-Fi interno | 14 centros de distribución | Sin failover; cortes registrados de 74 min |
| IoT sensores | AWS IoT Core | AWS | Funciona; sin integración de alertas al WMS |
...
| App Conductores (APP-15) | Recibe ruta y manifiesto en campo | AWS / DynamoDB | Recibe manifiestos parciales cuando WMS es lento |
...
- Los estados en el portal son inconsistentes con la realidad porque los eventos offline se sincronizan fuera de orden.
...
| App | Rol | Plataforma | Problema |
|---|---|---|---|
| App Conductores (APP-15) | Navegación, registro de entrega y evidencias | AWS / DynamoDB | Offline frágil; evidencias perdibles; motivos texto libre |
| S3 Evidencias (APP-16) | Almacena fotos y firmas de entrega | AWS S3 | Sin hash de integridad; evidencias sin garantía de completitud |
...
| Portal Trazabilidad Clientes (APP-18) | Visibilidad de estados para clientes B2B | SaaS externo | Muestra estados con retraso y a veces inconsistentes |
| Pasarela de Pagos (APP-17) | Procesa pagos contra entrega en campo | SaaS externo | Sin problemas reportados en el caso |
...
| Componente | Tecnología | Plataforma | Problema |
|---|---|---|---|
| App Conductores backend | AWS (servicio específico no indicado) | AWS | Sin mecanismo de retry robusto ante reinstalación |
| DynamoDB | AWS DynamoDB | AWS | No garantiza orden de eventos offline |
| S3 Evidencias | AWS S3 | AWS | Sin hash de integridad por archivo |
...
| App | Rol | Plataforma | Problema |
|---|---|---|---|
| App Conductores (APP-15) | Registra excepción con motivo y evidencia | AWS | Permite texto libre; motivos no comparables |
| TMS (APP-11) | Visualiza excepciones y planifica reintentos | Azure | Recibe categorías inconsistentes de la app |
| CRM Atención (APP-20) | Abre reclamos y gestiona contacto con cliente | SaaS externo | Taxonomía diferente a app y TMS |
| Portal Trazabilidad (APP-18) | Notifica excepción al cliente B2B | SaaS externo | Notificación con retraso |
| WMS (APP-06) | Recibe devolución cuando el pedido vuelve | On Premises | Sin integración automática desde la excepción |
...
#### Puntos de Dolor
- La conciliación entre nubes y sistemas locales toma hasta 23 días. Una cadena retail retuvo USD 2.4M porque sus reportes mostraban menos entregas exitosas que RutaExpress. La conciliación involucró archivos de AWS, reportes del TMS, capturas del portal y registros del WMS.
...
| Devolución | Pedidos que regresan al almacén | Estado en WMS; no siempre reflejado en portal o ERP |
...
| App | Rol | Plataforma | Problema |
|---|---|---|---|
| ERP Financiero (APP-25) | Facturación y liquidación principal | On Premises | Factura desde reportes mensuales; sin tiempo real |
...
| Dashboards / Reportes (APP-23) | Reportes para clientes y operaciones | GCP | Datos de la semana anterior |
| S3 Evidencias (APP-16) | Sustento de entrega para liquidación | AWS S3 | No integrado directamente con ERP |
| Portal Trazabilidad (APP-18) | Reportes de estado para cliente B2B | SaaS externo | Estados inconsistentes con ERP |
...
- Eliminar el canal de archivos CSV/S3; migrar clientes medianos a portal web con validación automática.
- Reducir defectos de ingreso de 6% a menos de 1%.

#### Roles
| Rol | Cambio TO BE |
|---|---|
| Cliente empresarial | Solo canal API o portal web con validación en tiempo real |
...
| Bus de Eventos (Event Hub / Kafka) | Desacopla orquestador de WMS; aplica backpressure y prioridad | Azure Event Hubs |
| Servicio de Notificación (pre-entrega) | Confirma ventana horaria con destinatario antes de procesar | AWS SNS o SaaS |
...
| App | Motivo |
|---|---|
| S3 Bucket legado (APP-04) | Canal de archivos a deprecar; clientes migran a API o portal |
| Portal carga manual CSV (APP-03) | Reemplazar por portal con validación automática integrada |
...
- Sincronización de inventario en tiempo real entre WMS y sistemas downstream (TMS, portal, ERP).
...
| Sistema WMS | Publica movimientos a Event Hub automáticamente |
...
| Temperatura | Alertas integradas con WMS para bloquear despacho fuera de rango |
...
| App | Descripción | Plataforma propuesta |
|---|---|---|
| WMS Cloud | WMS modernizado con auto-scaling, HA multi-zona y modo degradado | Azure (misma nube que TMS) |
...
| IoT Core sensores frío (APP-09) | Integrar alertas de temperatura con WMS para bloqueo automático de despacho | AWS IoT Core |

#### ELIMINAR
| App | Motivo |
|---|---|
| WMS on premises (APP-06) | Reemplazado por WMS cloud — migración progresiva por fases |
| WMS Satélite (APP-07) | Reemplazado por WMS cloud con modo degradado local |
...
| Componente | Tecnología TO BE | Cambio respecto AS IS |
|---|---|---|
| WMS | Cloud (Azure, contenedores con auto-scaling) | NUEVO — reemplaza on premises |
...
- Cerrar manifiestos solo con paquetes confirmados por el WMS.
...
| TMS | Cierra manifiesto solo cuando WMS confirma todos los paquetes |
...
| Manifiesto | Digital; cerrado solo con confirmación completa del WMS |
...
| Manifiesto Digital | Cierre digital de manifiesto con confirmación WMS | Azure AKS (parte del TMS) |

#### MODIFICAR
| App | Cambio | Plataforma |
|---|---|---|
| TMS (APP-11) | Integrar con Event Hub para recibir confirmaciones del WMS en tiempo real | Azure |
| App Conductores (APP-15) | Recibir manifiesto digital completo; actualizable durante la jornada | AWS |

#### ELIMINAR
| App | Motivo |
|---|---|
| Sistema Impresión Manifiestos (APP-14) | Reemplazado por manifiesto digital en app |
| Optimizador GCP batch (APP-12) | Reemplazado por optimizador en tiempo real |
...
- Tracking visible con estados siempre consistentes entre portal, app y TMS.
...
| Portal clientes | Muestra estado real del pedido en tiempo real |
...
| Estado del pedido | Único estado canónico visible en todos los sistemas (portal, TMS, app, CRM) |
...
| Servicio de Notificación al Destinatario | Confirma ventana horaria, envía link de seguimiento, alerta en tiempo real | AWS SNS o SaaS |

#### MODIFICAR
| App | Cambio | Plataforma |
|---|---|---|
| App Conductores (APP-15) | Rediseño de módulo offline: SQLite cifrado, retry robusto, evidencias atómicas, motivos normalizados obligatorios | AWS |
| S3 Evidencias (APP-16) | Agregar hash de integridad y política de retención por regulación | AWS S3 |
| Portal Trazabilidad (APP-18) | Conectar al Event Store canónico para mostrar estado real | SaaS / API |
...
| S3 Evidencias | AWS S3 + cifrado AES-256 + Object Lock | MODIFICAR — agregar integridad |
...
| Motor de Predicción de Excepciones | Detecta pedidos con alta probabilidad de fallo antes de salir a ruta | GCP (ML sobre datos históricos limpios) |

#### MODIFICAR
| App | Cambio | Plataforma |
|---|---|---|
| App Conductores (APP-15) | Taxonomía normalizada obligatoria; campos obligatorios por tipo de excepción | AWS |
| CRM Atención (APP-20) | Adoptar misma taxonomía canónica; integrar con Event Store | SaaS externo |
...
| App | Cambio | Plataforma |
|---|---|---|
| ERP Financiero (APP-25) | Agregar API de integración en tiempo real con el Servicio de Liquidación | On Premises (transitorio) |
| Dashboards / Reportes (APP-23) | Conectar a BigQuery streaming para reportes en tiempo real | GCP |

#### ELIMINAR
| App | Motivo |
|---|---|
| Sistema de Liquidación Excel (APP-26) | Reemplazado por el Servicio de Liquidación automatizado |
| Analítica batch semanal (APP-22) | Reemplazada por analítica en streaming |
...
| F2 | WMS on premises sin auto-scaling ni HA | 6h caído en Cyber Days; USD 1.1M penalidades | Tecnología |
| F2 | Sincronización horaria entre WMS satélites | 4,900 movimientos en conflicto | Datos |
...
| F3 | Rutas generadas sin confirmación completa del WMS | Manifiestos con paquetes faltantes | Datos |
| F3 | Manifiestos físicos en papel | Sin trazabilidad digital; deuda técnica | Negocio |
| F4 | Offline frágil en app conductores | 1,200 firmas perdidas; USD 2.4M retenidos | Aplicaciones |
...
| Sin modelo canónico de estados | Estados distintos en WMS, TMS, App y Portal | Datos |
