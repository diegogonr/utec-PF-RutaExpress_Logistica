...
> **Para el comité de arquitectura** — Topología **multinube AS IS**. **Plataformas de infraestructura:** On Premises (Lima), Cloud MS Azure (EEUU), Cloud AWS (EEUU), Cloud GCP (EEUU), Cloud SaaS - Software as a Service (EEUU). **Mensaje clave:** hoy las integraciones son P2P sin **Bus de Eventos Central (PLT-03)** Bus de Eventos Central; no hay **Plataforma de Observabilidad Unificada (PLT-01)** ni **Plataforma IaC (PLT-04)** desplegados. Propuesta TO BE → docs `09`, `10` y `11`.
...
**Brechas AS IS (plataformas no desplegadas):** Bus de Eventos Central (PLT-03) · Plataforma de Observabilidad Unificada (PLT-01) · Plataforma de Identidad y Accesos (IAM) (PLT-02) (parcial) · Plataforma IaC (PLT-04).
...
| Integraciones multinube | Punto a punto sin Bus de Eventos Central (PLT-03) (PLT-03) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) | Datos inconsistentes entre sistemas |
...
| Red almacenes (conectividad App Handhelds (picking) (APP-10) ↔ WMS Principal (On Premises) (APP-06)) | Wi-Fi sin redundancia | Pérdida de enlace con WMS Principal (On Premises) (APP-06) |
...
| **Cloud MS Azure (EEUU)** | **Red:** Internet/WAN (sin enlace dedicado)<br>**Servicios:** Azure API Management (APP-01), AKS<br>**Datos:** BD relacional Azure (suposición APP-02/APP-05/APP-11)<br>**Seguridad/Operación:** OAuth parcial en Azure API Management (APP-01) | Azure API Management (APP-01), Orquestador de Pedidos (APP-02), Validador de Pedidos (APP-05), TMS (Transportation Management) (APP-11), Portal Transportistas Tercerizados (APP-13) |
...
- **On Premises (Lima) ↔ Cloud MS Azure (EEUU):** integración directa P2P por WAN/Internet (sin Bus de Eventos Central (PLT-03) (PLT-03) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) central).
...
| On Premises (Lima) | ERP Financiero (On Premises) (APP-25), WMS Principal (On Premises) (APP-06) legacy | WMS Principal (On Premises) (APP-06), WMS Satélite (On Premises local) (APP-07), ERP Financiero (On Premises) (APP-25), Sistema de Liquidación (Excel) (APP-26) |
...
| Inconsistencia de datos entre nubes | Alta | Alto | Integraciones P2P sin Bus de Eventos Central (PLT-03) (PLT-03) de Eventos Central (PLT-03) (Bus de Eventos Central (PLT-03)) de Eventos Central (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) de Eventos Central (Bus de Eventos Central (PLT-03)) (Bus de Eventos Central (PLT-03)) de Eventos Central (Bus de Eventos Central (PLT-03)) central (Bus de Eventos Central (PLT-03) no existe) |
...
## 6. Infraestructura TO BE — topología recomendada (Alternativa A)

> **Alineación con Hito 2:** Azure como hub central de integración y gobierno; AWS para última milla y evidencias; GCP para optimización y analítica. Detalle de ADR y C4 en `HITO 2 - Requerimientos Y Diseño de Arquitectura de Solución/ARQUITECTURA_SOLUCION_TO_BE/`.

| Dominio | Plataforma | Componentes principales |
|---|---|---|
| Gobierno API, OMS centralizado / Orquestador de Pedidos (APP-02), inventario, TMS (Transportation Management) (APP-11) | **Cloud MS Azure (EEUU)** | Azure API Management (APP-01), Orquestador de Pedidos / OMS centralizado (APP-02), Inventario y Reservas, TMS (Transportation Management) (APP-11) (APP-11), WMS Cloud, Azure SQL, AKS |
| Bus de Eventos Central (PLT-03) y colas | **Cloud MS Azure (EEUU)** | **Bus de Eventos Central (PLT-03):** Azure Event Hubs + Azure Service Bus (DLQ, retry, replay, backpressure) |
| Última milla y evidencias | **Cloud AWS (EEUU)** | App de Conductores (APP-15), backend móvil store-and-forward, DynamoDB (outbox/estado móvil), S3+KMS (APP-16), puente SQS/EventBridge → Bus de Eventos Central (PLT-03) |
| Optimización y analítica | **Cloud GCP (EEUU)** | Optimizador dinámico, Pub/Sub, BigQuery, Vertex AI (APP-24) |
| Transversal | Multinube | Plataforma de Observabilidad Unificada (PLT-01) (OpenTelemetry + Azure Monitor), Plataforma de Identidad y Accesos (IAM) (PLT-02) (Entra ID + Key Vault), Plataforma IaC (PLT-04) (Terraform/pipelines) |
| Transición | On Premises / SaaS | WMS Principal (APP-06), WMS Satélite (APP-07), ERP Financiero (APP-25), portales y CRM por adaptadores |

**Principio:** no se crea un Event Store separado en AWS. Los eventos canónicos de orden, inventario, tracking, excepciones y liquidación transitan por **Bus de Eventos Central (PLT-03)** en Azure; AWS conserva el dominio móvil offline-first y las evidencias.
