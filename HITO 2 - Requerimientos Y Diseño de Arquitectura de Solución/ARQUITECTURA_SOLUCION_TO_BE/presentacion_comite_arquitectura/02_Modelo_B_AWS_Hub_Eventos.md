...
El Modelo B propone que AWS sea el centro principal de eventos, colas, ultima milla y resiliencia operativa. En este modelo, APP-02 tambien evoluciona a OMS centralizado, pero permanece en Azure junto con Azure API Management (APP-01). La diferencia principal es que los eventos corporativos se publican hacia AWS, donde Bus de Eventos Central (PLT-03) se implementa con EventBridge, SQS, workers, DLQ y replay.

AWS concentra de forma natural el backend movil, la operacion offline-first, las evidencias y el Bus de Eventos Central (PLT-03). Azure conserva el gobierno API, OMS centralizado / Orquestador de Pedidos (APP-02), TMS (Transportation Management) (APP-11)/adaptadores e identidad corporativa. GCP conserva optimizacion dinamica de rutas, analitica y modelos predictivos.

Mensaje ejecutivo:

> El Modelo B fortalece AWS como centro de ultima milla y eventos, pero exige mayor disciplina de gobierno cruzado porque OMS centralizado / Orquestador de Pedidos (APP-02) y API governance permanecen en Azure mientras el Bus de Eventos Central (PLT-03) se desplaza a AWS.

## Alcance cubierto

| Iniciativa | Como la cubre el Modelo B |
|---|---|
| INI-01 Gestion unificada de ordenes e inventario | APP-02 evoluciona a OMS centralizado / Orquestador de Pedidos (APP-02) en Azure AKS; publica eventos de orden e inventario hacia AWS EventBridge/SQS. |
| INI-02 Integracion API-first y event-driven | Azure API Management (APP-01) gobierna APIs; AWS EventBridge/SQS implementa Bus de Eventos Central (PLT-03), eventos, colas, DLQ, replay y backpressure. |
...
| INI-06 Conciliacion financiera | Estados de orden, tracking, evidencias y eventos se auditan en AWS y se integran con ERP Financiero (On Premises) (APP-25) desde Azure/adapter. |
...
| Azure | APIs, OMS centralizado / Orquestador de Pedidos (APP-02), TMS (Transportation Management) (APP-11)/adaptadores, repositorio transaccional, identidad y gobierno de contratos. | Azure API Management (APP-01), AKS, Azure SQL, Entra ID, Key Vault, Azure Monitor. |
| GCP | Optimizacion, analitica, modelos predictivos y tableros avanzados. | Cloud Run/GKE, Pub/Sub, Dataflow, BigQuery, Vertex AI. |
| On premises / SaaS | Sistemas transicionales y externos. | WMS Principal (On Premises) (APP-06) APP-06/APP-07, ERP Financiero (On Premises) (APP-25) APP-25, Portal/CRM, canales legados. |
...
| Sistemas externos | WMS Principal (On Premises) (APP-06), TMS (Transportation Management) (APP-11), ERP Financiero (On Premises) (APP-25), Portal/CRM, legados y servicios de mapas/trafico. |
| Flechas | Relaciones funcionales. En este nivel no se muestra aun que AWS sea el Bus de Eventos Central (PLT-03); eso se observa en Nivel 2. |
...
5. La plataforma intercambia inventario, rutas, valorizacion y trazabilidad con WMS Principal (On Premises) (APP-06), TMS (Transportation Management) (APP-11), ERP Financiero (On Premises) (APP-25) y portal/CRM.
...
Este nivel responde a la pregunta: **como se reparte la plataforma en aplicaciones, servicios ejecutables, buses, colas y repositorios de datos cuando AWS es el Bus de Eventos Central (PLT-03)**.
...
| OMS centralizado / Orquestador de Pedidos (APP-02) e Inventario APP-02 | Gestiona ordenes, validacion, deduplicacion, reservas, liberaciones y estado operacional en Azure. |
| Azure SQL | Repositorio transaccional de OMS centralizado / Orquestador de Pedidos (APP-02) e inventario. |
| AWS EventBridge Bus de Eventos Central (PLT-03) | Hub principal de eventos, ruteo, fan-out, reglas y distribucion por dominio. |
...
2. API Management valida contrato, seguridad, cuotas y politicas antes de llegar al OMS centralizado / Orquestador de Pedidos (APP-02) en Azure.
3. El OMS centralizado / Orquestador de Pedidos (APP-02) valida la orden, aplica idempotencia, deduplicacion, reservas y estado operacional.
4. El OMS centralizado / Orquestador de Pedidos (APP-02) publica OrderEvents e InventoryEvents hacia AWS EventBridge.
...
8. Adaptadores consumen eventos para TMS (Transportation Management) (APP-11), portal/CRM, legados y procesos de soporte.
...
El hub operativo de eventos queda en AWS. Esto acerca eventos, colas y ultima milla, pero separa el Bus de Eventos Central (PLT-03) del OMS centralizado / Orquestador de Pedidos (APP-02) y del gobierno API, que permanecen en Azure.

## C4 Nivel 3 - Componentes del Bus de Eventos Central (PLT-03) en AWS

![Modelo B - C4 Nivel 3 Componentes](../diagramas_c4/imagenes_python_graphviz/alternativa_B_n3_componentes.png)

### Como leer el diagrama

Este nivel responde a la pregunta: **como funciona internamente el contenedor critico Bus de Eventos Central (PLT-03) cuando AWS es el hub principal de eventos**.

| Componente | Objetivo |
|---|---|
| Event Ingestion | Recibe eventos desde OMS centralizado / Orquestador de Pedidos (APP-02)/Inventario en Azure, backend movil AWS y adaptadores legados. |
...
1. Azure API Management (APP-01) gobierna contratos antes de que OMS centralizado / Orquestador de Pedidos (APP-02) publique eventos.
2. OMS centralizado / Orquestador de Pedidos (APP-02)/Inventario envia eventos de orden e inventario hacia AWS.
...
|---|---|
| API-first | Azure API Management (APP-01) conserva gobierno de APIs, contratos, versionado, cuotas y mocks. |
...
| Riesgo | Mitigacion |
|---|---|
| OMS centralizado / Orquestador de Pedidos (APP-02) y API governance quedan separados del Bus de Eventos Central (PLT-03). | Bridge Azure-AWS con contratos versionados, outbox, idempotencia, circuit breaker y monitoreo especifico. |
...
- AWS se acepta como hub principal de eventos Bus de Eventos Central (PLT-03).
- Azure mantiene API Management, OMS centralizado / Orquestador de Pedidos (APP-02), TMS (Transportation Management) (APP-11) y repositorio transaccional.
