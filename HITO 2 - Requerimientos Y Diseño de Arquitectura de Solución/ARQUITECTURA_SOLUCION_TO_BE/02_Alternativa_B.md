...
La **Alternativa B** consolida eventos, colas, backend movil, store-and-forward y evidencias en **AWS**. APP-02 tambien evoluciona a un **OMS centralizado**, pero permanece en Azure junto con Azure API Management (APP-01) y el repositorio transaccional.

La diferencia principal respecto al Modelo A es que Bus de Eventos Central (PLT-03) se ubica en AWS, usando EventBridge, SQS y workers para ruteo, colas, DLQ, retry, replay y backpressure. GCP se conserva para optimizacion dinamica, analitica y modelos predictivos.
...
**Riesgo principal:** OMS centralizado / Orquestador de Pedidos (APP-02) y gobierno API quedan separados del Bus de Eventos Central (PLT-03), por lo que el puente Azure -> AWS se vuelve componente critico.
...
|---|---|
| **Integracion** | API-first en Azure API Management (APP-01); Event-Driven Architecture con Bus de Eventos Central (PLT-03) en AWS; puente Azure -> AWS para eventos de OMS centralizado / Orquestador de Pedidos (APP-02)/Inventario. |
...
| **Datos** | Azure SQL para estado OMS centralizado / Orquestador de Pedidos (APP-02)/Inventario; DynamoDB logico y S3/KMS para ultima milla; BigQuery para analitica. |
...
|---|---|
| **Hub-and-Spoke federado** | AWS EventBridge como Bus de Eventos Central (PLT-03); OMS centralizado / Orquestador de Pedidos (APP-02) Azure, backend movil, TMS (Transportation Management) (APP-11), portal y GCP como productores/consumidores. |
...
| **Outbox/Inbox** | Publicacion confiable desde OMS centralizado / Orquestador de Pedidos (APP-02) en Azure hacia EventBridge en AWS. |
...
|---|---|---|
| Orquestador de Pedidos (APP-02) | **Evoluciona a OMS centralizado / Orquestador de Pedidos (APP-02) e Inventario en Azure** | Mantiene gobierno API y estado transaccional cerca de Azure. |
| WMS Principal / Satelite (APP-06 / APP-07) | **Integracion desde OMS centralizado / Orquestador de Pedidos (APP-02)/Inventario en Azure** | Conserva transicion, pero publica eventos hacia AWS. |
| Integraciones punto a punto | **Reemplazo progresivo por hub AWS** | EventBridge/SQS asumen fan-out, retry, DLQ y replay. |
| App de Conductores (APP-15) | **Fortalecida en AWS** | Queda cerca del Bus de Eventos Central (PLT-03), backend movil y evidencias. |
...
| **Azure** | API Management, OMS centralizado / Orquestador de Pedidos (APP-02), TMS (Transportation Management) (APP-11)/adaptadores, Azure SQL, identidad corporativa. | Mantiene APP-02 y gobierno API en su eje actual. |
...
| OMS centralizado / Orquestador de Pedidos (APP-02) e Inventario APP-02 | Azure | AKS; ordenes, validacion, deduplicacion, reservas, liberaciones y conciliacion. |
| Repositorio transaccional | Azure | Azure SQL; ordenes, inventario y estado operacional. |
| Adaptador TMS (Transportation Management) (APP-11) | Azure | Integracion con APP-11 para despacho y rutas. |
| Hub principal de eventos Bus de Eventos Central (PLT-03) | AWS | EventBridge; eventos canonicos, reglas, fan-out y ruteo. |
...
**Flujo clave:** Cliente -> API Management -> OMS centralizado / Orquestador de Pedidos (APP-02) Azure -> EventBridge AWS -> SQS/workers -> TMS (Transportation Management) (APP-11), portal/CRM, backend movil y GCP.
...
Componentes principales **Bus de Eventos Central (PLT-03) en AWS**:

- **Event Ingestion:** recibe eventos desde OMS centralizado / Orquestador de Pedidos (APP-02)/Inventario, backend movil y legados.
...
|---|---|
| INI-01 ordenes e inventario | OMS centralizado / Orquestador de Pedidos (APP-02)/Inventario en Azure, Azure SQL, publicacion hacia AWS, idempotencia y conciliacion WMS Principal (On Premises) (APP-06)/ERP. |
| INI-02 API-first/event-driven | Azure API Management (APP-01), AWS EventBridge, SQS, DLQ, replay, backpressure y contratos canonicos. |
...
| INI-06 conciliacion | Evidencias, estados, tracking, OMS centralizado / Orquestador de Pedidos (APP-02), TMS (Transportation Management) (APP-11), ERP Financiero (On Premises) (APP-25) y auditoria de eventos federada. |
...
| Decision | Implementar Bus de Eventos Central (PLT-03) en AWS con EventBridge, SQS y workers. |
...
### ADR-B-002 - APP-02 permanece en Azure como OMS centralizado / Orquestador de Pedidos (APP-02)

| Campo | Decision |
|---|---|
| Estado | Propuesto |
| Contexto | Hito 1 ya ubica APP-02 en el eje Azure/API governance. |
| Decision | APP-02 evoluciona a OMS centralizado / Orquestador de Pedidos (APP-02) en Azure y publica eventos hacia AWS. |
| Consecuencias | Menor cambio para OMS centralizado / Orquestador de Pedidos (APP-02); mayor necesidad de bridge confiable. |
| Alternativas descartadas | Migrar OMS centralizado / Orquestador de Pedidos (APP-02) a AWS durante el MVP. |
...
----------------              ---------------------          ----------------
Bus de Eventos Central (PLT-03) EventBridge            Azure API Management (APP-01)           Pub/Sub analitico
SQS colas / DLQ / replay      OMS centralizado / Orquestador de Pedidos (APP-02) APP-02 en AKS              Cloud Run/GKE rutas
Lambda/ECS validators         Azure SQL                      Dataflow
Backend movil ECS/Lambda      Adaptador TMS (Transportation Management) (APP-11)                  BigQuery
...
| Riesgo | Mitigacion |
|---|---|
| Bridge Azure -> AWS se vuelve critico. | Outbox en OMS centralizado / Orquestador de Pedidos (APP-02), retry, DLQ, circuit breaker, health checks y monitoreo de latencia. |
...
| Inconsistencia OMS centralizado / Orquestador de Pedidos (APP-02)/EventBridge. | Idempotencia, ordering guard, reconciliacion y auditoria de eventos. |
